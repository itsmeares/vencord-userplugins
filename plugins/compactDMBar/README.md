...
- Discord keeps ownership of guild-width resizing while CompactDMBar handles the Home sidebar through its native handle;
- dragging 24 px right from compact snaps to the remembered normal Home width, then keeps resizing in the same drag;
- dragging 24 px left past the normal minimum snaps to compact, and reversing direction works without releasing;
- double-clicking the native handle toggles compact/default mode;
- the compact/default choice and independent Home/guild widths persist through Vencord settings.

The plugin deliberately keeps Discord's existing navigation, account profile flow and account actions mounted. Runtime DOM work locates the active sidebar, preserves Discord's native guild resizing and adds a small reversible resize gesture only for the Home compact/default boundary. Account controls are integrated through the same account-panel React patch pattern used by Vencord's built-in plugins.
